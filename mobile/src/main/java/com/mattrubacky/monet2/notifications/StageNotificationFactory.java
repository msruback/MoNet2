...
public class StageNotificationFactory extends NotificationFactory {
    ...
    private ArrayList<Notification> getFromTimePeriod(ArrayList<TimePeriod> timePeriods, ArrayList<com.mattrubacky.monet2.deserialized.splatoon.StageNotification> toNotify){
        ArrayList<Notification> notifications = new ArrayList<>();
        if(timePeriods!=null) {
            for (int i = 0; i < timePeriods.size(); i++) {
                TimePeriod timePeriod = timePeriods.get(i);
                for (int j = 0; j < toNotify.size(); j++) {
                    com.mattrubacky.monet2.deserialized.splatoon.StageNotification notification = toNotify.get(j);
