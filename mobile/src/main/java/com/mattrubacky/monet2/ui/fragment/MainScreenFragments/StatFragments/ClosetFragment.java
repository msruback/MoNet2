...
import com.mattrubacky.monet2.ui.adapter.RecyclerView.GearPagerAdapter;
...
public class ClosetFragment extends Fragment {
    ...
    private ArrayList<GearStats> gearList;
    ...
    private void updateUi(){
        ArrayList<ArrayList<GearStats>> gearLists = new ArrayList<>();
        ArrayList<GearStats> head = new ArrayList<>(),clothes = new ArrayList<>(),shoes = new ArrayList<>();
        GearStats gearStats;
        ...
        for(int i=0;i<gearList.size();i++){
            gearStats = gearList.get(i);
            switch(gearStats.gear.kind){
                case "head":
                    head.add(gearStats);
                    break;
                case "clothes":
                    clothes.add(gearStats);
                    break;
                case "shoes":
                    shoes.add(gearStats);
                    break;
    ...
    private ArrayList<GearStats> sort(ArrayList<GearStats> data){
        if(data.size()<=1){
            return data;
        }
        if(data.size()==2){
            if(data.get(0).gear.id<=data.get(1).gear.id){
                ...
                GearStats hold = data.get(0);
                data.remove(0);
                data.add(hold);
                return data;
            }
        }else{
            GearStats pivot = data.get(0);
            ArrayList<GearStats> lower = new ArrayList<>();
            ArrayList<GearStats> upper = new ArrayList<>();
            ...
            ArrayList<GearStats> result = sort(lower);
            result.add(pivot);
