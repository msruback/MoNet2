...
public class HeadManager {
    ...
    public void insert(){
        ...
        if(toInsert.size()>0) {
            ...
            String whereClause = SplatnetContract.Head._ID + " = ?";
            String[] args;
            Cursor cursor = null;
    ...
    public Gear select(int id){
        ...
        Cursor cursor = database.query(SplatnetContract.Head.TABLE_NAME,null,whereClause,args,null,null,null);
        ...
        if(cursor.moveToFirst()){
            ...
            gear.id = cursor.getInt(cursor.getColumnIndex(SplatnetContract.Head._ID));
            gear.name = cursor.getString(cursor.getColumnIndex(SplatnetContract.Head.COLUMN_NAME));
            gear.kind = cursor.getString(cursor.getColumnIndex(SplatnetContract.Head.COLUMN_KIND));
            gear.rarity = cursor.getInt(cursor.getColumnIndex(SplatnetContract.Head.COLUMN_RARITY));
            gear.url = cursor.getString(cursor.getColumnIndex(SplatnetContract.Head.COLUMN_URL));

            brandID = cursor.getInt(cursor.getColumnIndex(SplatnetContract.Head.COLUMN_BRAND));
    ...
    public HashMap<Integer,Gear> select(){
        ...
        if(toSelect.size()>0) {
            ...
            for (int i = 1; i < toSelect.size(); i++) {
                builder.append(" OR " + SplatnetContract.Head._ID + " = ?");
    ...
    public ArrayList<Gear> selectAll(){
        ...
        Cursor cursor = database.query(SplatnetContract.Head.TABLE_NAME,null,null,null,null,null,null);
